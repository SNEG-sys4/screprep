...
(function (global) {
  ...
  function applyFilters(){
    ...
    // renderAll()（#t1〜#t7 の再描画）では一切触れられない。よって
    // buildAiExcludeUI() をここで再呼び出しする必要はない（チェックボックスの
    // イベントリスナーは loadFromStore() 時に1回登録したまま維持される）。
    // 以前ここで毎回 buildAiExcludeUI() を呼んでいたが、フィルタ変更のたびに
    // 100件規模のチェックリストDOMを丸ごと作り直す無駄な処理だったため削除。
  }
