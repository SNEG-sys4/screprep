...
(function (global) {
  ...
  function ctx(){
    ...
    return { k, ir, gs, wc, ln,
      ...
      web: App.ac_f.filter(r=>r['Webアクセス'] && !isNil(r['リスク分類'])),
  ...
  async function buildHtmlReport(){
    ...
    const cards=[
      ...
      card(c.gsL,'② Webアクセス：ガバナンス健全度',`全Web ${fmtInt(tw)}件 ／ 高リスク ${fmtInt(hr)}件 中リスク ${fmtInt(mr)}件`,c.gs,'点',
  ...
  async function buildWordReport(secs, cmts){
    ...
    if(secs.web_risk){ children.push(para(new PageBreak())); children.push(heading('2. リスク遮断・Webガバナンス分析',RED)); children.push(para(run(' ')));
      ...
      children.push(insightCard('🟡','SNS・AI外部サービス',mr>0?'yellow':'green',
        `中リスクアクセス ${fmtInt(mr)}件を検出。生成AI等への機密情報入力リスクがあります。`,'利用ガイドラインを整備し全社周知を推奨します。','中リスクアクセス',fmtInt(mr)+'件'));
  ...
  global._exports = { buildHtmlReport, buildWordReport, buildExcelReport, figToPng };
